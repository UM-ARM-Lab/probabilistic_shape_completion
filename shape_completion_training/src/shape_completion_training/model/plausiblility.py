...
def compute_icp_fit_dict(metadata):
    ...
    with progressbar.ProgressBar(widgets=widgets, max_value=num_shapes) as bar:
        for i, reference in ds.enumerate():
            bar.update(i.numpy())
            # bar.update(i, CurrentShape=data_tools.get_unique_name(reference))

            best_fit_for_reference = {}
            # print("Fitting To: {}".format(data_tools.get_unique_name(reference)))
            ...
            for other in ds:
                ...
                best_fit_for_reference[other_name] = T
